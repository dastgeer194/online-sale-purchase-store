...
<script>
	...
	function syncCheckoutFields($form)
	{
		var fieldMap = {
			customer_name: $('#checkout_customer_name'),
			customer_phone: $('#checkout_customer_phone'),
			customer_email: $('#checkout_customer_email'),
			customer_address: $('#checkout_customer_address'),
			customer_city: $('#checkout_customer_city')
		};
		var labels = {
			customer_name: 'Name',
		...
		for (var key in fieldMap) {
			...
			var value = $.trim(fieldMap[key].val());
			$form.find('input[name="' + key + '"]').val(value);

			if (labels[key] && value === '') {
				alert(labels[key] + ' is required.');
				fieldMap[key].focus();
				return false;
			}
		}

		if (fieldMap.customer_email.length) {
			var email = $.trim(fieldMap.customer_email.val());
			if (email !== '') {
				var emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
				if (!emailPattern.test(email)) {
					alert('Please provide a valid email address.');
					fieldMap.customer_email.focus();
					return false;
				}
			}
		}
	...
	$(document).on('submit', '#paypal_form, #bank_form, #cod_form', function () {
		return syncCheckoutFields($(this));
	});
